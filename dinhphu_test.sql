...
-- =============================
-- 6) Validation queries (Q1/2026)
-- =============================
-- 6.1 Kiem tra Pending payment khong co dong ledger
SELECT
  p.`PaymentId`,
  p.`Status`,
  COUNT(fl.`LedgerId`) AS `LedgerRows`
FROM `Payment` p
LEFT JOIN `FinancialLedger` fl ON fl.`ReferenceId` = p.`PaymentId`
WHERE p.`PaymentId` = 'PMT9100006'
GROUP BY p.`PaymentId`, p.`Status`;

-- 6.2 Kiem tra moi Refund tao dung 2 dong EventType
SELECT
  fl.`ReferenceId`,
  SUM(CASE WHEN fl.`EventType` = 'RefundPayout' THEN 1 ELSE 0 END) AS `RefundPayoutRows`,
  SUM(CASE WHEN fl.`EventType` = 'PenaltyRevenue' THEN 1 ELSE 0 END) AS `PenaltyRevenueRows`
FROM `FinancialLedger` fl
WHERE fl.`ReferenceId` IN ('RFD9100001', 'RFD9100002')
GROUP BY fl.`ReferenceId`;

-- 6.3 Tong doanh thu vao va tien hoan theo khach san (Q1)
SELECT
  x.`HotelId`,
  ROUND(SUM(CASE WHEN x.`EventType` IN ('Deposit', 'FinalPayment', 'FullPayment', 'PenaltyRevenue') THEN x.`AmountIn` ELSE 0 END), 2) AS `TotalRevenueIn`,
  ROUND(SUM(CASE WHEN x.`EventType` = 'RefundPayout' THEN x.`AmountOut` ELSE 0 END), 2) AS `TotalRefundPayout`
FROM (
  SELECT
    rm.`HotelId`,
    fl.`EventType`,
    COALESCE(fl.`DebitAmount`, 0) AS `AmountIn`,
    COALESCE(fl.`CreditAmount`, 0) AS `AmountOut`
  FROM `FinancialLedger` fl
  ...
  WHERE p.`Status` = 'Completed'
    AND YEAR(fl.`Date`) = 2026
    AND QUARTER(fl.`Date`) = 1
